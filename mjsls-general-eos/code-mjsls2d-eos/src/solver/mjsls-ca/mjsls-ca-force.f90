...
subroutine cal_node_force_ca(nx,ny,amass,pre1,pre1_in,vertx,verty,xc,yc, &
   ...
   ! --------------------------------
	...
	do i = 1 , nx - 1
      ...
      do k = 1 , 8
         ...
         ! if( i == nx/2) then 
         ! print*, '730', (pre1(k1,k2,k3)+pre1_in(k4,k2,k3)),alength,anormaly
         ! endif            
      enddo    
         ! if( i == nx/2) then 
         ! print*, '----------', i,j, fy_pre(i,j),pre1(4,i+1,j+1),pre1(4,i+1,j)
         ! endif          
	enddo
	enddo
   ! pause 248
   ...
   ! ----*----b2-[i,j]--b1---*-------
	...
	do i = 1 , nx - 1
      ...
      do k = 1 , 4
         ...
         fx_pre(i,j) = fx_pre(i,j) + (pre1(k1,k2,k3)+pre1_in(k4,k2,k3))*alength*anormalx 
         fy_pre(i,j) = fy_pre(i,j) + (pre1(k1,k2,k3)+pre1_in(k4,k2,k3))*alength*anormaly 
         ! print*, i,j,(pre1(k1,k2,k3)+pre1_in(k4,k2,k3))*alength*anormaly ; pause
      enddo      
...
end subroutine cal_node_force_ca
