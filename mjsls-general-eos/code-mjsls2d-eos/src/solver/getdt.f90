...
subroutine getdt0(sound,amass,den,vertx,verty,Vstarx,Vstary,nx,ny,cfl,dt,it)
 implicit none 
   ...
   do j=1,ny
      ...
      dAdt=dAdt+(ur*yr1+vr1*xr-ur1*yr-vr*xr1)*0.5d0 
      !c2
      ir=i
      jr=j-1
      ir1=i
      ...
      tV=min(CV*amass(i,j)/den(i,j)/(abs(dAdt)+1.d-10),tV)  
   ...
   enddo
      ...
      ! if(dt > 1.d-3)  dt = 1.d-4
      ! if(dt < 1.d-6)  dt = 1.d-6
      !  dt = 2.d-4
