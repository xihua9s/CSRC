!> 20250226
!> xihua
!> initial acousticpulse problem
subroutine initial_acousticpulse_ca(supt,den,pre,sound,enin,eos, &
   ...
   real*8  :: subcell_volm_cy_aw(1:nx,1:ny,1:4) 
	...
	real*8 :: PI,xl,xr,yd,yu,dx,dy,r,theta,xr1,yr1,xr2,yr2,lambda,x,xoff,w 
	integer :: i,j
   ...
   vstary = 0.d0    
	do  i=1,nx 
	do  j=1,ny    
      x = (vertx(i-1,j) + vertx(i,j))/2.d0
      if( x-xoff >= 0.d0 .and. x-xoff <= lambda ) then 
         w = 0.01d0*(1.d0-cos(2.d0*pi/lambda*(x-xoff)))
      else 
         w = 0.d0 
      endif 
		den(i,j) = 1.d0 + w 
...
end subroutine initial_acousticpulse_ca
