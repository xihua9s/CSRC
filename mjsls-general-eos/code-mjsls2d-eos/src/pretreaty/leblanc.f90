...
subroutine initial_leblanc_rectangle_ca(supt,den,pre,sound,enin,eos, &
   ...
   real*8  :: subcell_volm_cy_aw(1:nx,1:ny,1:4)    
	...
	real*8 :: PI,xl,xr,yd,yu,dx,dy,r,theta,xr1,yr1,xr2,yr2,xc
	integer :: i,j
   ...
   gamar = 5.d0 / 3.d0 
	...
	do  i=1,nx 
	do  j=1,ny   
		xc = ( vertx(i-1,j) + vertx(i,j)) / 2.d0
		if( xc < 3.d0) then  
			den(i,j) = 1.d0
			eos(i,j) = 513
			pre(i,j) = 2.d0/3.d0 * 0.1d0
		else 
			den(i,j) = 1.d-3
			eos(i,j) = 513
			pre(i,j) = 2.d0/3.d0 * 1.d-10
		endif 
...
end subroutine initial_leblanc_rectangle_ca
