...
Subroutine solver_staggered_Landshoff(ini_cmt,mesh_phys)
	implicit none
	...
	Do While (ini_cmt%current_time < ini_cmt%Time_stop)
		...
		Call solver_staggered_Landshoff_details( &
			ini_cmt%nx,      &
			ini_cmt%dt,      &
...
end Subroutine solver_staggered_Landshoff

Subroutine solver_staggered_Landshoff_details(nx,dt,den,pre,sos,ein,mass,eos,&
	...
	do ix = 1 , nx 
		if( vel(ix+1) < vel(ix) ) then
			pre1(ix) = pre(ix) + vis1*den(ix)*sos(ix)*abs(vel(ix+1)-vel(ix)) + &
	...
	do ix = 1 , nx 
		if( vel_pre(ix+1) < vel_pre(ix) ) then
			pre2(ix) = pre_pre(ix) + &
						vis1*den_pre(ix)*sos_pre(ix)*abs(vel_pre(ix+1)-vel_pre(ix)) + &
...
end Subroutine solver_staggered_Landshoff_details

end module mdu_Landshoff  
