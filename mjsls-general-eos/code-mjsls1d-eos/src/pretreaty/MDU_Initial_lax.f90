!> Last Modified : 四  3/ 2 20:28:42 2023

Module MDU_Initial_lax
	use common_inicmt
	use common_meshphys
...
Contains
	...
	Subroutine Initial_Mesh_Phys_lax(ini_cmt,mesh_phys)
		implicit none
		...
		ini_cmt%time_stop = 0.16d0
		dx = 1.d0 / (ini_cmt%nx + 0.d0)

		do ix = 1 , ini_cmt%nx + 1
		mesh_phys%vertex(ix) = (ix - 1) * dx
		if ( mesh_phys%vertex(ix) < 0.5d0) then
			mesh_phys%vel_p(ix) = 0.698d0
		else 
		...
		mesh_phys%bnd_vel_pre(1) = 0.698d0 
		mesh_phys%bnd_vel_pre(2) = 0.d0 

	End Subroutine Initial_Mesh_Phys_lax


End Module MDU_Initial_lax
